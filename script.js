...
(function(){
  ...
  var CATEGORY_LABELS = {beads:'Beads', chains:'Chains', findings:'Findings', stones:'Stones', wire:'Wire', tools:'Tools'};
  function categoryLabel(cat){ return CATEGORY_LABELS[cat] || 'Beads'; }
  function fmtMoney(n){
    n = Number(n)||0;
  ...
  async function init(){
    ...
    if(!items){
      items = [
        {id: uid(), name:'Kundan', img:'', color: colorFor('Kundan'), qty:12, price:100, unit:'pc', cat:'beads', desc:'', sku:'', unitLabel:''},
        {id: uid(), name:'Chain', img:'', color: colorFor('Chain'), qty:34, price:100, unit:'pc', cat:'chains', desc:'', sku:'', unitLabel:''},
        {id: uid(), name:'Beads', img:'', color: colorFor('Beads'), qty:56, price:100, unit:'packet', cat:'beads', desc:'', sku:'', unitLabel:''}
      ];
  ...
  function openItemModal(item){
    state.modal = {type:'item', payload: item ? Object.assign({}, item) : {id:null, name:'', img:'', qty:0, price:0, unit:'pc', cat:'beads', desc:'', sku:'', unitLabel:''}};
    render();
  }
  async function submitItemModal(formEl){
    ...
    var cat = (fd.get('cat')||'beads').toString();
    ...
    if(payload.id){
      ...
      if(it){
        it.name = name; it.qty = qty; it.price = price; it.img = img; it.unit = unit;
        it.cat = cat; it.desc = desc; it.sku = sku; it.unitLabel = unitLabelText;
        it.color = colorFor(name);
      }
    } else {
      newlyCreatedItem = {id: uid(), name:name, img:img, color: colorFor(name), qty:qty, price:price, unit:unit, cat:cat, desc:desc, sku:sku, unitLabel:unitLabelText};
  ...
  function triggerQuickAddItem(lineId, name){
    ...
    openItemModal({id:null, name:name, img:'', qty:0, price:0, unit:'pc', cat:'beads', desc:'', sku:'', unitLabel:''});
  }
  ...
  function renderItems(){
    ...
    var rows = filteredItems.map(function(i){
      ...
      return '<tr>' +
        ...
        '<td class="item-name">'+esc(i.name)+'<div class="muted">'+categoryLabel(i.cat)+(i.sku?' &middot; '+esc(i.sku):'')+'</div></td>' +
        '<td>'+i.qty+' '+unitLabel(i.unit)+' '+(out?'<span class="pill low" style="margin-left:8px;">out of stock</span>':(low?'<span class="pill low" style="margin-left:8px;">low</span>':''))+'</td>' +
          ...
          '<div class="modal" onclick="event.stopPropagation()">' +
            ...
            '<form id="item-form">' +
              ...
              '<div class="field"><label>Website Category</label><select name="cat">' +
                '<option value="beads" '+((it.cat||'beads')==='beads'?'selected':'')+'>Beads</option>' +
                '<option value="chains" '+(it.cat==='chains'?'selected':'')+'>Chains</option>' +
                '<option value="findings" '+(it.cat==='findings'?'selected':'')+'>Findings</option>' +
                '<option value="stones" '+(it.cat==='stones'?'selected':'')+'>Stones</option>' +
                '<option value="wire" '+(it.cat==='wire'?'selected':'')+'>Wire</option>' +
                '<option value="tools" '+(it.cat==='tools'?'selected':'')+'>Tools</option>' +
              '</select></div>' +
              '<div class="field"><label>Website Pack Text <em style="text-transform:none;font-weight:400;color:#9A8C7C;">(optional, e.g. "per 50g pack")</em></label><input name="unitLabel" type="text" value="'+esc(it.unitLabel||'')+'" placeholder="Leave blank to auto-use the unit above"></div>' +
              '<div class="field"><label>SKU <em style="text-transform:none;font-weight:400;color:#9A8C7C;">(optional)</em></label><input name="sku" type="text" value="'+esc(it.sku||'')+'" placeholder="Auto-generated if left blank"></div>' +
              '<div class="field"><label>Website Description <em style="text-transform:none;font-weight:400;color:#9A8C7C;">(optional)</em></label><input name="desc" type="text" value="'+esc(it.desc||'')+'" placeholder="Shown on the product card on the website"></div>' +
              ...
              '<p class="field-hint">Leave picture blank to use an automatic monogram badge instead. Set Quantity to 0 to automatically mark it "Out of Stock" on the website.</p>' +
    ...
    if(state.modal.type === 'confirmDeleteItem'){
      ...
      return (
        '<div class="modal-backdrop" data-close-modal>' +
          '<div class="modal" onclick="event.stopPropagation()">' +
            '<h2>Delete Item?</h2>' +
            '<p class="muted">This removes "'+esc(item?item.name:'')+'" from inventory and the website. Past invoices are not affected.</p>' +
            '<div class="modal-actions">' +
